...
with open_po as (
  select sku, unit, sum(open_po_quantity) as open_po_qty
    from vyndi_open_purchase_orders
   group by sku, unit
),
committed as (
...
with receipt_state as (
  select purchase_order_id,
         coalesce(sum(quantity_quarantined), 0) as quantity_quarantined
...
select
  ...
  case
    when coalesce(r.quantity_quarantined, 0) > 0
      then 'PENDING_INSPECTION_OR_QUARANTINE'
    when p.status in ('approved', 'issued', 'part_received') and coalesce(p.quantity_accepted, 0) < p.quantity
      then 'OPEN_RECEIPT'
    when p.status = 'draft' then 'UNISSUED_DRAFT'
    when p.status = 'pending_approval' then 'AWAITING_APPROVAL'
    else 'OK'
  end as exception_class
from vyndi_purchase_order_status p
left join receipt_state r on r.purchase_order_id = p.id
where p.status in ('draft', 'pending_approval', 'approved', 'issued', 'part_received')
   or coalesce(r.quantity_quarantined, 0) > 0;
...
select
  ...
  c.variant_id,
  case
    when c.model_tier = 'core' then 'aluminium'
    when c.model_tier in ('pro', 'apex') then 'carbon'
    else null
  end as venture,
  jsonb_array_length(c.released_mapping_set) as released_mapping_count,
  case
    when c.bom_revision is null or c.bom_revision = '' then 'MISSING_BOM_REVISION'
    when jsonb_array_length(c.released_mapping_set) = 0 then 'NO_RELEASED_MAPPING'
    when exists (
      select 1
        from jsonb_array_elements_text(c.released_mapping_set) released(mapping_id)
        left join epr_bom_inventory_mappings m on m.id = released.mapping_id
       where m.id is null
          or m.bom_revision is distinct from c.bom_revision
          or m.status <> 'active'
          or m.approved_by is null
          or m.approved_at is null
          or m.effective_from > now()
          or (m.effective_to is not null and m.effective_to <= now())
    ) then 'INVALID_RELEASED_MAPPING'
    else 'OK'
  end as compliance
...
comment on view vyndi_report_bom_compliance is
  'Job cards whose released BOM mapping snapshot is missing, stale or no longer approved/active. Report-only.';
...
select
  ...
  o.plan_month,
  case
    when c.status = 'planned' then 'AWAITING_RELEASE'
...
select
  ...
  case
    ...
    when o.status = 'cancelled' and c.status <> 'cancelled' then 'CANCELLED_ORDER_ACTIVE_JOB'
    else 'SYNCED'
  end as sync_status
...
select
  ...
  p.invoice_on,
  p.due_on,
  (current_date - p.due_on) as age_days,
  case
    when p.due_on < current_date - 90 then 'OVER_90'
    when p.due_on < current_date - 60 then 'OVER_60'
    when p.due_on < current_date - 30 then 'OVER_30'
    when p.due_on < current_date then 'OVERDUE'
    else 'CURRENT'
  end as aging_bucket,
  case when p.status = 'blocked' then 'BLOCKED' else 'OPEN' end as payable_class
from vyndi_accounts_payable p
where p.status <> 'void'
  and p.amount_open_inr > 0;

comment on view vyndi_report_payables_aging is
  'Open supplier invoices from canonical accounts payable, aged from due date. Report-only.';
...
select
  ...
  (select count(*)::int from epr_production_job_cards where status not in ('complete', 'cancelled')) as open_job_cards,
  ...
  (select count(*)::int from vyndi_purchase_order_status where status in ('approved', 'issued', 'part_received')) as open_pos,
