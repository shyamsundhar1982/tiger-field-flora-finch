...
-- Fixed column/status alignment against live schema (reservations.reserved_at,
-- PO statuses draft/pending_approval/approved/issued/part_received/received/cancelled).
-- Report-only: never posts, deletes, reverses or mutates business truth.
...
with open_po as (
  select sku, vyndi_canonical_unit(unit) as unit,
   ...
   where status in ('approved', 'issued', 'part_received', 'pending_approval')
...
select
  ...
  case
    ...
    when p.status in ('issued', 'part_received') and coalesce(p.quantity_received, 0) < p.quantity
      then 'OPEN_RECEIPT'
    when p.status in ('draft', 'pending_approval') then 'UNISSUED_DRAFT'
    else 'OK'
...
select
  ...
  c.venture,
  case
    when c.bom_revision is null or trim(c.bom_revision) = '' then 'MISSING_BOM_REVISION'
    when not exists (
...
select
  ...
  o.plan_month,
  case
    when c.status in ('draft', 'pending', 'pending_release', 'awaiting_approval') then 'AWAITING_RELEASE'
    when c.status = 'released' then 'RELEASED'
    when c.status in ('in_progress') then 'IN_PROGRESS'
...
select
  ...
  case
    ...
    when o.status = 'cancelled' and coalesce(c.status, 'cancelled') not in ('cancelled') then 'CANCELLED_ORDER_ACTIVE_JOB'
    else 'SYNCED'
  end as sync_status
...
select
  ...
  case
    when p.status in ('draft', 'pending_approval') then 'UNISSUED'
    when p.status in ('approved', 'issued', 'part_received') then 'OPEN'
    else upper(p.status)
...
select
  ...
  (select count(*)::int from vyndi_purchase_order_status
    where status in ('draft', 'pending_approval', 'approved', 'issued', 'part_received')) as open_pos,
