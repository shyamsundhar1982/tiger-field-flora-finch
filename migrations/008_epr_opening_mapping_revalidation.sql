-- Phase 7 hardening: revalidate the approved BOM -> Inventory Master mapping
-- at both opening approval and authoritative posting time.
-- This prevents a previously-created opening from bypassing a retired mapping.
...
begin
  ...
  -- The mapping must still be active at the exact moment the authoritative
  -- transaction is posted. The mapping itself was only approvable against an
  -- approved Inventory Master record.
  if not exists (
    select 1
    from epr_bom_inventory_mappings m
    join master_data_records md
      on md.domain='inventory' and md.code=m.sku and md.status='approved'
    ...
    raise exception 'Opening balance requires an active approved BOM-SKU mapping backed by an approved Inventory Master SKU.';
  end if;
  ...
  if exists (
    select 1 from epr_inventory_opening_balances
