-- Reconcile PR #42 production automation with PR #44 procurement-cost authority.
-- Restores the dependent open-PO view after the compatibility shim and replaces
-- the production approval function so automatic draft POs never use catalogue /
-- legacy reference prices as procurement cost.
...
begin
  ...
  loop
    ...
    -- PR #44 authority only: FIFO actual -> approved PO -> approved supplier ->
    -- approved planning procurement price -> explicit missing. Catalogue / legacy
    -- reference price is deliberately excluded from governed_cost_inr.
...
comment on function approve_vyndi_production_batch(text,text,text) is
  'One governed bike/batch release. Auto draft POs use vyndi_procurement_cost_authority only; legacy/catalogue reference prices are never procurement cost.';
