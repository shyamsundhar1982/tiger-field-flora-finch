-- Fix PL/pgSQL output-variable/column ambiguity in the canonical Commercial demand writer.
-- Keep the public function signature unchanged because the application expects
-- (sales_order_id, revision, created), but qualify table-column references that
-- share names with those OUT columns.
...
begin
  ...
  insert into vyndi_sales_order_revisions as sor
  ...
  on conflict on constraint vyndi_sales_order_revisions_sales_order_id_revision_key
  do nothing;

  if not exists (
    select 1
      from vyndi_sales_order_revisions as sor
     where sor.sales_order_id = p_id
       and sor.revision = v_revision
  ) then
    raise exception 'Sales-order revision receipt was not persisted.';
  end if;

  insert into vyndi_audit_events as vae
