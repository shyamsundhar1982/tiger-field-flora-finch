...
const target = process.env.VIBPE_UI_TARGET || new URL(baseUrl || "http://localhost").host || "local";

if (!baseUrl) {
  console.error("VIBPE_UI_BASE_URL is required, e.g. https://your-deployment.example");
  process.exit(2);
}
...
async function checkRoute(capabilityId, routePath, label) {
  ...
  try {
    ...
    const loginLike = /\/login(?:[/?#]|$)/i.test(finalUrl) || /sign in|log in/i.test(bodyText.slice(0, 500));
