...
const input = {
  ...
  inventory:[{sku:"SKU-1",onHandQty:10,reservedQty:2,safetyStockQty:2,mslQty:2,unitCostLakh:0.01,sourceRef:"ATP"}],
...
test("IBPE engine is deterministic and governed run persistence is idempotent/advisory", async (t) => {
  const pg=await db(); t.after(()=>pg.close());
  const first=runIntegratedBusinessPlanningEngine(input,{horizonMonths:36});
  const second=runIntegratedBusinessPlanningEngine(input,{horizonMonths:36});
  assert.deepEqual(second,first,"same governed input must reproduce the same IBPE output");
  ...
  const args=["IBPE-TEST-1","VYNDI-IBPE-1.0.0","abcdef0123456789",inputHash,"PLAN-TEST",1,"base",new Date("2026-09-08T00:00:00Z").toISOString(),JSON.stringify(input),JSON.stringify(first),JSON.stringify({gate:"test"}),"tester","operations"];
  ...
  const runRows=await pg.query(`select input_hash,approved_plan_revision,status from vyndi_ibpe_runs`);
  assert.equal(runRows.rows.length,1); assert.equal(runRows.rows[0].input_hash,inputHash); assert.equal(Number(runRows.rows[0].approved_plan_revision),1); assert.equal(runRows.rows[0].status,"complete");
  ...
  assert.equal(Number(salesAfter.rows[0].count),Number(salesBefore.rows[0].count),"persisting IBPE output must not write transaction truth");
});
...
test("IBPE persistence rejects a non-approved plan revision", async (t) => {
  ...
  await assert.rejects(() => pg.query(
    ...
    ["IBPE-DRAFT","VYNDI-IBPE-1.0.0","abcdef0123456789",hash(input),"PLAN-DRAFT",1,"base",new Date().toISOString(),JSON.stringify(input),JSON.stringify({}),JSON.stringify({}),"tester","operations"],
