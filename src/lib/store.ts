...
type Store = {
  ...
  updateGlobalFinance: (key: keyof Omit<FinanceAssumptions, "productLines" | "aluminiumVertical">, value: number) => void;
...
export const useVeloxis = create<Store>((set) => ({
  ...
  updateGlobalFinance: (key, value) => set((st) => ({ finance: { ...st.finance, [key]: value } })),
  updateProductLine: (id, key, value) => set((st) => ({
    finance: { ...st.finance, productLines: st.finance.productLines.map((line) => line.id === id ? { ...line, [key]: value } : line) },
  })),
  updateAluminiumVertical: (key, value) => set((st) => ({
    finance: { ...st.finance, aluminiumVertical: { ...st.finance.aluminiumVertical, [key]: value } },
  })),
  resetFinance: () => set({ finance: DEFAULT_FINANCE_ASSUMPTIONS }),
}));
