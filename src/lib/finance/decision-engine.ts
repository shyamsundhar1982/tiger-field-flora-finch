...
export function sensitivity(finance: FinanceAssumptions, accounting: AccountingAssumptions, drawStandby: boolean, field: keyof DecisionOverrides, deltas = [-10, -5, 5, 10]) {
  return deltas.map((delta) => buildDecisionResult({ id: "custom", label: `${delta > 0 ? "+" : ""}${delta}%`, description: `Sensitivity: ${String(field)}`, overrides: { ...DEFAULT_DECISION_OVERRIDES.base, [field]: delta } }, finance, accounting, drawStandby));
}
