...
const meta = (
  ...
  maturity: PageMaturity,
  group: NavigationGroup,
): RouteMeta => ({ route, label, mode, domain, owner, maturity, group });

/**
 * Phase B registry: every route currently exposed by the command-shell sidebar
 * is classified here. The existing sidebar remains unchanged; this registry is
 * the source of truth for future metadata-driven navigation views.
 */
...
for (const page of Object.values(routeRegistry)) {
  navigationGroups[page.group].push(page);
}
