...
async function activeReleasedMappings(
  ...
  order: OrderRow,
): Promise<{ bomRevision: string; rows: MappingRow[] }> {
  ...
  const rows = await sql.query<MappingRow>(
    ...
    [venture, order.variant_id],
  );
  if (!rows.length) {
    throw new Error(`No active approved BOM mapping exists for ${order.variant_id}. Release the variant BOM before Production.`);
  }
  const revisions = [...new Set(rows.map((row) => row.bom_revision))];
  if (revisions.length !== 1) {
    throw new Error(`Production release is ambiguous: ${order.variant_id} has ${revisions.length} active BOM revisions (${revisions.join(", ")}). Supersede all but one.`);
  }
  ...
  for (const item of configured) {
    const approved = selected.some(
      (mapping) =>
        mapping.sku === item.sku &&
        mapping.configuration_option_id === item.id,
    );
    if (!approved) {
  ...
  return { bomRevision, rows: selected };
}

/**
 * Synchronize the controlled Production projection from the current central Sales
 * order revision. The client cannot supply units, month, status, BOM or SKU lines.
 */
export const syncProductionJobCard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [existing] = await sql.query<{ id: string; status: string; sales_order_revision: number | string }>(
      `select id,status,sales_order_revision from epr_production_job_cards where sales_order_id=$1 limit 1`,
      [order.id],
    );
    ...
    if (order.status === "delivered") {
      if (!existing || existing.status !== "complete") {
        throw new Error("An order cannot be treated as delivered until its production job card is complete.");
      }
      return { id: existing.id, created: false, synchronized: true, state: "complete" as const };
    }

    if (!order.model_tier || !order.variant_id) {
      throw new Error("Confirmed production demand requires model tier and exact variant.");
    }
    const { bomRevision, rows: mappings } = await activeReleasedMappings(sql, order);
    ...
    await sql.query(
      ...
      [
        `AUD-JC-${crypto.randomUUID()}`,
        jobId,
        Number(order.revision),
        existing ? "synchronized_to_sales_revision" : "released_from_sales_order",
        actor.userId,
        actor.role,
        order.id,
        JSON.stringify({ variantId: order.variant_id, bomRevision, mappingIds: mappings.map((m) => m.id), units: Number(order.units), dueMonth: Number(order.plan_month) }),
      ],
    );

    return { id: jobId, created: !existing, synchronized: true, state: "released" as const, salesOrderRevision: Number(order.revision), bomRevision };
  });
...
export const refreshProductionReservations = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const actor = await requireBusinessActor("edit");
    const sql = await getSql();
    const lines = await sql.query<{ id: string }>(
      `select id from epr_production_job_card_lines where job_card_id=$1 and sku is not null order by id`,
      [data.jobCardId],
    );
...
export const issueProductionReservation = createServerFn({ method: "POST" })
  .validator(
    z.object({
      reservationId: z.string().min(1).max(160),
      travellerId: z.string().min(1).max(160),
    }),
  )
  .handler(async ({ data }) => {
    const actor = await requireBusinessActor("edit");
...
export const getProductionJobCards = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const lines = await sql`
    select job_card_line_id as id,job_card_id,sku,category,item,required_quantity as quantity,unit,
     ...
     order by job_card_id,job_card_line_id
  `;
  return { cards, lines };
});
