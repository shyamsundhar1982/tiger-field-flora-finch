...
const rowToItem = (r: Record<string, unknown>): InventoryItem => ({
  ...
  coreEnabled: Boolean(r.core_enabled),
  proEnabled: Boolean(r.pro_enabled),
  apexEnabled: Boolean(r.apex_enabled),
    ...
    const rows = await sql.query<Record<string, unknown>>(
      `insert into component_inventory
       ...
       on conflict (id) do update set
         ...
         model=excluded.model, detail=excluded.detail, sku=excluded.sku, price_inr=excluded.price_inr,
         stock_qty=excluded.stock_qty, reorder_level=excluded.reorder_level,
         core_enabled=excluded.core_enabled, pro_enabled=excluded.pro_enabled,
         apex_enabled=excluded.apex_enabled, source=excluded.source, notes=excluded.notes, updated_at=now()
