...
export type AdvancedPlanningAuthorityAssessment = {
  ...
  supplierLaneAuthority: "not-compiled" | "approved-persisted";
...
export function buildAdvancedPlanningFromGovernedIbpe(
  ...
  const authority: AdvancedPlanningAuthorityAssessment = {
    ...
    supplierLaneAuthority: hasPersistedSupplierLanes ? "approved-persisted" : "not-compiled",
    persistedSupplierLaneRevisionIds: hasPersistedSupplierLanes
      ? [...(source.persistedSupplierLaneRevisionIds ?? [])].sort()
      : undefined,
    firmCtpEligible: false,
    optimisationEligible: false,
    limitations: [
      ...
      ...(hasPersistedSupplierLanes
        ? ["Supplier-lane lead time, order policy, landed cost, governed reliability evidence and finite capacity come from approved persisted supplier+SKU lane revisions."]
      ...
      "Firm CTP still depends on committed inventory/receipt truth; supplier-lane remediation is never promoted into a customer promise automatically.",
