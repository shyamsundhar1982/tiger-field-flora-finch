...
const cash: CashPlanRow[] = [
  {
    ...
    closingCashLakh: 10,
    freeLiquidityLakh: 8,
    closingCashAfterRecommendationsLakh: 10,
    freeLiquidityAfterRecommendationsLakh: 8,
  },
  {
    period: 2,
    selectedInflowsLakh: 5,
    selectedOutflowsLakh: 2,
    incrementalProcurementLakh: 0,
    closingCashLakh: 13,
    freeLiquidityLakh: 11,
    closingCashAfterRecommendationsLakh: 13,
    freeLiquidityAfterRecommendationsLakh: 11,
  },
  {
    period: 3,
    selectedInflowsLakh: 0,
    selectedOutflowsLakh: 4,
    incrementalProcurementLakh: 0,
    closingCashLakh: 9,
    freeLiquidityLakh: 7,
    closingCashAfterRecommendationsLakh: 9,
    freeLiquidityAfterRecommendationsLakh: 7,
  },
];
...
test("cash guardrail detects a future reserve breach caused by earlier procurement spend", () => {
  ...
  assert.equal(result.periods[0].proposedProcurementBreachesHeadroom, false);
  assert.equal(result.periods[2].proposedProcurementBreachesHeadroom, true);
});
...
test("baseline reserve breach is preserved even with zero incremental procurement", () => {
  ...
  assert.equal(compiled.valid, true);
  assert.ok(compiled.issues.some((row) => row.code === "BASELINE_RESERVE_BREACH" && row.period === 2));
  assert.equal(result.status, "infeasible");
  assert.equal(result.firstBaselineBreachPeriod, 2);
});

test("missing cash authority or supplier lane stays indeterminate rather than becoming zero cost or unlimited cash", () => {
  const incomplete = compileCashGuardrailsFromIbpe(cash.slice(0, 2), 3, "IBPE-RUN-102:CASH");
  assert.equal(incomplete.valid, false);
  assert.ok(incomplete.issues.some((row) => row.code === "MISSING_CASH_PERIOD" && row.period === 3));
