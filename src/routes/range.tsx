...
function Range() {
  ...
  if (pathname !== "/range") return <Outlet />;
  ...
  const tierGroupsets = tierId === "core"
  ...
  const buildAdd = useMemo(() => {
    ...
    return { groupset: g, tyre: ty, wheelset: w, groupsetAdj: delta(g, tierGroupsets, defaults.groupset), tyreAdj: delta(ty, TYRES, defaults.tyre), wheelsetAdj: delta(w, WHEELSETS, defaults.wheelset) };
  }, [groupset, tyre, wheelset, defaults, tierGroupsets]);
