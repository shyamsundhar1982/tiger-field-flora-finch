...
function Range() {
  ...
  return <div className="min-h-dvh bg-bg"><SiteHeader /><main className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
    ...
    <section className="mt-16 rounded-2xl border border-accent/25 bg-gradient-to-br from-accent/12 via-green/8 to-transparent p-5 shadow-[0_0_50px_rgb(255_116_23/0.05)] sm:p-7">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-green">Full build configurator</p><h2 className="group mt-1 cursor-default text-3xl font-bold text-accent transition-all duration-300 hover:text-shadow-orange">Configure {selectedTier.name}</h2><p className="mt-2 max-w-2xl text-sm text-muted">The published bicycle price is the base specification. Component choices apply only their upgrade or downgrade difference, keeping the configurator aligned with the product price.</p></div><div className="rounded-xl border border-accent/40 bg-bg/90 px-5 py-3 text-right shadow-[0_0_22px_rgb(255_116_23/0.08)]"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-green">Configured build</p><p className="mt-1 text-2xl font-bold tabular-nums text-accent">{inr(buildPrice)}</p><p className="mt-0.5 text-[10px] text-subtle">Base + option adjustments</p></div></div>
