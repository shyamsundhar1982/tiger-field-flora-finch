...
function BomControl() {
  ...
  async function refresh() {
    try {
      const [a, c] = await Promise.all([listMasterData(), listEprMappings()]);
    ...
    try {
      await createMasterData({
        data: {
          ...
          attributes: { venture: b.venture, modelId: b.model, controlState: "controlled" },
        },
      });
      setB({ ...b, code: "", name: "" });
      await refresh();
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 space-y-6">
      ...
      <Panel title="Component register" kicker={`${maps.length} mappings`}>
        <div className="overflow-x-auto">
            ...
            <tbody>
              {maps.map((x) => (
                <tr key={x.id} className="border-t border-border/70">
                  ...
                  <td>
                    {x.status === "draft" && (
                      <button onClick={() => void approve(x.id)} className="text-accent text-xs">
                        Approve
                      </button>
                    )}
                  </td>
      ...
      <div className="text-xs text-muted">Existing editable BOM cost engine remains available for costing; this page is the controlled identity/requirement layer used by inventory execution.</div>
    </main>
  );
}
