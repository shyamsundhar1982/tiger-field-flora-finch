...
function InvestorBoardPage() {
  ...
  return <div className="space-y-8">
    <header>
      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-green">Investor / Board · read-only</p>
      <h1 className="mt-2 font-display text-3xl font-semibold tracking-tight text-accent">Capital, diligence & decisions</h1>
      ...
      <div className="mt-3 flex flex-wrap gap-3 text-sm font-semibold"><Link to="/command/financial-cockpit" className="text-accent">Finance evidence →</Link><Link to="/command/qa-verification" className="text-accent">QA evidence →</Link><Link to="/command/governance" className="text-accent">Governance register →</Link><Link to="/command/epr-live" className="text-accent">EPR evidence →</Link></div>
    </header>
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      ].map(([label, value]) => <div key={label} className="rounded-lg border border-border bg-surface p-4"><p className="text-[10px] uppercase tracking-widest text-green">{label}</p><p className="mt-2 text-xl font-semibold">{value}</p></div>)}
    </section>

    <section className="rounded-lg border border-border bg-surface p-5">
      <h2 className="text-sm font-semibold text-accent">Investor thesis / proof points</h2>
    ...
    <section>
      <div className="mb-3 flex items-end justify-between"><div><h2 className="text-sm font-semibold text-accent">Capital ladder</h2><p className="text-xs text-muted">Each tranche is conditional on the stated evidence gate.</p></div></div>
    ...
    <section>
      <h2 className="mb-3 text-sm font-semibold text-accent">Use of proceeds</h2>
      <div className="grid gap-3 md:grid-cols-5">{USE_OF_PROCEEDS.map((item) => <div key={item.tranche} className="rounded-lg border border-border p-4"><p className="text-xs font-semibold text-green">{item.tranche}</p><p className="mt-2 text-sm font-medium">{item.allocation}</p><p className="mt-2 text-xs text-muted">{item.note}</p></div>)}</div>
    </section>

    <section>
      <h2 className="mb-3 text-sm font-semibold text-accent">Board decision register</h2>
    ...
    <section>
      <div className="mb-3 flex items-end justify-between"><div><h2 className="text-sm font-semibold text-accent">Investor diligence room</h2><p className="text-xs text-muted">{readyDiligence} of {DILIGENCE_ITEMS.length} categories currently marked ready.</p></div></div>
    ...
    <section className="grid gap-4 md:grid-cols-2">
      <div className="rounded-lg border border-border bg-surface p-5"><h2 className="text-sm font-semibold text-accent">Next investor actions</h2><ol className="mt-4 space-y-3 text-sm text-muted">{INVESTOR_NEXT_ACTIONS.map((action, index) => <li key={action} className="flex gap-3"><span className="text-xs text-subtle">0{index + 1}</span><span>{action}</span></li>)}</ol></div>
      <div className="rounded-lg border border-border bg-surface p-5"><h2 className="text-sm font-semibold text-accent">Fundraise rule</h2><p className="mt-4 text-sm leading-6 text-muted">{INVESTOR_BOARD_STATUS.strategy}</p><div className="mt-4 rounded-md border border-border p-3 text-xs text-muted">Investor-facing claims must remain aligned with controlled Finance, QA, EPR and Governance evidence. Pending validation must never be presented as completed validation.</div></div>
    </section>
  </div>;
}
