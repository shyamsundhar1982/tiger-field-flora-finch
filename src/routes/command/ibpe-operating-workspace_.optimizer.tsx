  ...
  async function execute() {
    ...
    try {
      ...
      const fundingDependency = response.cashPlanningDisposition === "funding-required"
        && response.minimumAdditionalFundingLakh !== null
        && response.fundingRequiredByPeriod !== null
        ? ` Planning disposition funding-required: minimum additional funding ₹${response.minimumAdditionalFundingLakh}L by period ${response.fundingRequiredByPeriod}. Execution remains blocked until funding is evidenced.`
        : "";
      setMessage(
        ...
        {state.recentRun ? (
          <>
            {state.recentRun.cash_planning_disposition === "funding-required" ? (
              <div className="mb-4 rounded-lg border border-warn/30 bg-warn/5 p-4">
                ...
                <p className="mt-1 text-xs leading-5 text-muted">
                  ...
                  Minimum additional funding: {fundingText(state.recentRun.minimum_additional_funding_lakh)} by period {state.recentRun.funding_required_by_period ?? "—"}.
            ...
            <div className="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-3">
              ...
              <div><p className="text-xs text-muted">Minimum additional funding</p><p className="mt-1 text-fg">{fundingText(state.recentRun.minimum_additional_funding_lakh)}</p></div>
              <div><p className="text-xs text-muted">Funding required by</p><p className="mt-1 text-fg">{state.recentRun.funding_required_by_period ? `Period ${state.recentRun.funding_required_by_period}` : "—"}</p></div>
