...
function LedgerPage() {
  ...
  const ledger = (INVENTORY_LEDGER_PAGES.some((page) => page.id === rawLedger) ? rawLedger : "stock") as InventoryLedgerId;
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6">
    <header className="flex flex-col gap-4"><div><p className="text-[10px] uppercase tracking-[0.22em] text-green">Master ledger · {page.group}</p><h1 className="mt-2 text-3xl font-bold text-accent sm:text-4xl">{page.label}</h1><p className="mt-2 max-w-4xl text-sm leading-6 text-muted">{page.detail}. Each row is a purchase lot or asset record; issue actions consume the oldest available lot first.</p></div><InventoryWorkspaceNav active={ledger} /></header>
