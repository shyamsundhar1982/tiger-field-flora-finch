...
function DemoCompany() {
  return <div className="space-y-7">
    <header>
      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-green">VYNDI OS · Showcase Mode</p>
      <h1 className="mt-1 font-display text-4xl text-accent">Demo Company</h1>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">A realistic end-to-end company scenario for demonstrating VYNDI OS without presenting demonstration data as audited company performance. The scenario shows how one business moves from signal to decision, execution, evidence and stakeholder reporting.</p>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Kpi label="Product platform" value="VYNDI" hint="Connected operating system"/><Kpi label="Capital story" value="₹15L → ₹2Cr" hint="Staged funding ladder"/><Kpi label="Planning horizon" value="36 mo" hint="Evidence-gated execution"/><Kpi label="Control principle" value="Evidence" hint="No silent completion"/></div>

    <section className="rounded-xl border border-border bg-surface p-6 sm:p-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-green">The demo company story</p><h2 className="mt-3 max-w-4xl font-display text-3xl leading-tight text-accent">A performance-bicycle company uses VYNDI OS to connect product creation with cash, production, compliance and governance.</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-muted">The demonstration begins with a business signal and follows it through the complete operating loop. Each layer remains accountable to its owner while the command layer preserves the executive picture.</p></section>
