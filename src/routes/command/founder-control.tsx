...
const money = (n: number | undefined) => `₹${(Number.isFinite(n) ? Number(n) : 0).toFixed(1)}L`;
    ...
    <div className="space-y-6">
      ...
      <Panel title="Intervention rules" kicker="Founder guardrails">
        <div className="grid gap-3 md:grid-cols-2">
          {FOUNDER_CONTROL_CHECKS.map((check) => (
            <Link key={check.id} to={check.route as never} className="rounded-lg border border-border bg-surface p-4 hover:border-accent">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-accent">{check.id}</span>
                <span className={`text-[10px] uppercase tracking-wider ${check.severity === "critical" ? "text-danger" : "text-warn"}`}>{check.severity}</span>
              </div>
              <p className="mt-2 text-sm font-medium">{check.title}</p>
              <p className="mt-1 text-xs text-muted">Source: {check.source}</p>
              <p className="mt-2 text-xs text-fg">Action: {check.action}</p>
            </Link>
          ))}
        </div>
      </Panel>
      ...
      <Panel title="Operating lanes" kicker="Owner → rule → control">
        <div className="space-y-2">
          {FOUNDER_CONTROL_LANES.map((lane) => (
            <div key={lane.id} className="flex flex-col gap-2 rounded-lg border border-border bg-surface p-4 md:flex-row md:items-center">
              <div className="md:w-44">
                <p className="text-sm font-medium">{lane.title}</p>
                <p className="text-[10px] uppercase tracking-wider text-subtle">Owner: {lane.owner}</p>
              </div>
              <p className="flex-1 text-xs leading-5 text-muted">{lane.rule}</p>
              <Link to={lane.route as never} className="text-xs text-accent">
                Open control →
              </Link>
            </div>
          ))}
      ...
      <Panel title="Action urgency" kicker="Derived from controlled queue">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody>
              {founderActionSummary.map((a) => (
                <tr key={a.id} className="border-t border-border">
                  <td className="py-3 pr-3 text-accent">{a.id}</td>
                  <td className="py-3 pr-3">{a.urgency}</td>
                  <td className="py-3 pr-3 font-medium">{a.title}</td>
                  <td className="py-3 pr-3 text-muted">{a.owner}</td>
                  <td className="py-3 pr-3">{a.status}</td>
                  <td className="py-3">
                    <Link to={a.controlRoute as never} className="text-xs text-accent">
                      Open →
                    </Link>
                  </td>
                </tr>
      ...
      <div className="rounded-lg border border-border bg-surface p-5">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-1 text-lg font-semibold">{FOUNDER_CONTROL_THRESHOLDS.evidenceFreshnessDays} days</p>
          </div>
          <div>
            <p className="text-xs text-muted">Critical action horizon</p>
