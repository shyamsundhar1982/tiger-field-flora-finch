...
function ScenarioStudio() {
  ...
  return <div className="space-y-6">
    ...
    {packet ? <>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-accent/25 bg-accent/8 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg border border-accent/25 bg-bg/50 text-accent"><GitCompareArrows className="size-5" /></span><div><p className="font-medium text-fg">Explore this result with IBPE Copilot</p><p className="mt-1 text-xs leading-5 text-muted">The global Copilot now has <strong className="text-fg">{packet.scenario.label}</strong> as its scenario context. Ask why a number changed, what drives a shortage, or which controlled action improves feasibility.</p></div></div><span className="shrink-0 text-xs font-medium text-accent">Open “IBPE Copilot” →</span></div>
    </> : null}
  </div>;
}
