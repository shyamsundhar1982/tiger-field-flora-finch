      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
            One screen for the five questions that matter: <span className="text-fg">where is the money, where is it going, what are we getting back, when do we run out, and what changes the answer?</span>
          </p>
        </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-3">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] uppercase tracking-[0.16em] text-subtle">Live scenario</span>
            {scenarios.map((item) => (
              <button key={item.id} type="button" onClick={() => setScenario(item.id)} className={`rounded-md border px-3 py-1.5 text-xs ${scenario === item.id ? "border-accent bg-accent text-bg" : "border-border text-muted hover:text-fg"}`}>
                {item.label}
              </button>
            ))}
          </div>
        ...
        <div className="grid gap-2 md:grid-cols-7">
          {[
            ...
            <div key={label} className="relative rounded-lg border border-border bg-surface p-3">
              ...
              <p className="mt-2 text-[10px] uppercase tracking-[0.12em] text-subtle">{label}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Panel title="Portfolio volumetrics" kicker="Units behind the financial result">
          <div className="space-y-3">
            {[['Aluminium', productUnits.aluminium], ['Carbon', productUnits.carbon], ['Premium Carbon', productUnits.premium]].map(([label, units]) => {
              const n = Number(units); const share = t.units ? (n / t.units) * 100 : 0;
