      ...
      <Panel title="Today's Control Room" kicker="Exception-first management view">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Kpi
            ...
            value={number(data.todaysControlRoom.actionRequired)}
            hint="Rows requiring management attention"
            tone={data.todaysControlRoom.actionRequired ? "warn" : "ok"}
          />
          <Kpi
            label="Unverified areas"
            value={number(data.todaysControlRoom.unverified)}
            hint="Empty evidence is never treated as healthy"
            tone={data.todaysControlRoom.unverified ? "warn" : "ok"}
          />
        ...
        {data.todaysControlRoom.attentionAreas.length ? (
          <div className="mt-4 flex flex-wrap gap-2">
            {data.todaysControlRoom.attentionAreas.map((area) => (
              <span key={area} className="rounded-full border border-warn/30 bg-warn/5 px-3 py-1 text-xs text-warn">
                {area}
              </span>
            ))}
          </div>
          ...
          {data.sections.map((section) => (
            <article key={section.key} className="rounded-xl border border-border bg-surface/30 p-4">
              ...
              <div className="mt-4 flex items-center justify-between text-xs">
                ...
                <Link to={section.authorityRoute as never} className="font-semibold text-accent hover:underline">
                  Open authority →
                </Link>
              </div>
            </article>
