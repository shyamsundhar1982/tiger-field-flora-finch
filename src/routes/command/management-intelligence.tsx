...
function ManagementIntelligence(){
  ...
  return <div className="space-y-6">
    <header><p className="text-[11px] uppercase tracking-[0.22em] text-green">VINDY · Management read model</p><h1 className="mt-1 font-display text-4xl text-accent">Management Intelligence</h1><p className="mt-2 max-w-4xl text-sm leading-6 text-muted">Read-only decision intelligence connecting sales, production, inventory, funding, cash and engineering readiness. Management-model figures remain assumptions until evidence is reconciled.</p><div className="mt-3 flex flex-wrap gap-3 text-sm font-semibold"><Link to="/command/inventory-truth" className="text-accent">Inventory truth →</Link><Link to="/command/financial-cockpit" className="text-accent">Financial cockpit →</Link><Link to="/command/actuals" className="text-accent">Operational actuals →</Link></div></header>
