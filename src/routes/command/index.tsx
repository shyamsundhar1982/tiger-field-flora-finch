...
const STAGES = [
  ...
  { n: 4, label: "Financial Planning Control · 36M", to: "/command/financial-cockpit" as const },
...
const FINANCE_WORKSPACES = [
  { label: "Financial Cockpit", to: "/command/financial-cockpit" as const, note: "Cash safety, funding, burn, gross profit, break-even and the 36-month trajectory" },
  { label: "Plan & Assumptions", to: "/command/finance-assumptions" as const, note: "Single source of truth for ASP, COGS, volume, mix, launch, opex, capex and funding" },
  { label: "Scenarios", to: "/command/scenarios" as const, note: "Base, Delayed and Stress compared side-by-side without changing the underlying model" },
  { label: "Master Financial Dashboard", to: "/command/master-finance" as const, note: "Consolidated Aluminium + Carbon production, sales, gross profit, funding, capex, inventory and cash" },
...
const OPERATIONS_WORKSPACES = [
  { label: "Inventory Planning", to: "/command/inventory" as const, note: "Inventory as cash: reorder exposure, planned purchase draw, stock position and runway connection" },
  { label: "Component Control", to: "/inventory" as const, note: "Components, stock, reorder levels, tier eligibility and configuration availability" },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted">The command layer now puts financial planning first: one live model, one assumptions surface, three scenarios and connected operating workspaces.</p>
      </div>

      <Panel title="Financial planning first" kicker="Start here">
        <div className="grid gap-3 md:grid-cols-3">
          <Link to="/command/financial-cockpit" className="rounded-xl border border-accent bg-accent/10 p-4"><p className="text-sm font-semibold text-fg">1 · Financial Cockpit</p><p className="mt-2 text-xs leading-5 text-muted">Are we safe? Cash, burn, funding, margin, break-even.</p></Link>
          <Link to="/command/finance-assumptions" className="rounded-xl border border-border bg-surface p-4 hover:border-accent"><p className="text-sm font-semibold text-fg">2 · Plan & Assumptions</p><p className="mt-2 text-xs leading-5 text-muted">What can we change? ASP, COGS, volume, opex, capex.</p></Link>
          <Link to="/command/scenarios" className="rounded-xl border border-border bg-surface p-4 hover:border-accent"><p className="text-sm font-semibold text-fg">3 · Scenarios</p><p className="mt-2 text-xs leading-5 text-muted">What happens if timing or costs move?</p></Link>
        </div>
      </Panel>

      <Panel title="12-stage execution roadmap" kicker="Command architecture">
        <div className="rounded-xl border border-border bg-bg-elevated/30 p-3"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{STAGES.map((stage)=><Link key={stage.n} to={stage.to} activeOptions={{exact:stage.n===1}} className="group flex items-start gap-3 rounded-lg border border-border bg-bg-elevated/95 p-3 transition-colors duration-200 hover:border-accent/45 hover:bg-bg" activeProps={{className:"bg-bg border-accent"}}><span className="flex size-7 shrink-0 items-center justify-center rounded-full border border-border text-xs tabular-nums text-accent">{stage.n}</span><span className="min-w-0"><span className="block text-[10px] uppercase tracking-[0.14em] text-subtle">Stage {stage.n}</span><span className="mt-0.5 block text-sm font-medium text-fg group-hover:text-accent">{stage.label}</span></span></Link>)}</div></div>
      </Panel>
      ...
      <Panel title="Operations & market intelligence" kicker="Controlled internal workspaces"><div className="rounded-xl border border-border bg-bg-elevated/30 p-3"><div className="grid gap-3 md:grid-cols-3">{OPERATIONS_WORKSPACES.map((page)=><Link key={page.to} to={page.to} className="group rounded-lg border border-border bg-bg-elevated/95 p-4 transition-colors duration-200 hover:border-accent/45 hover:bg-bg"><p className="text-sm font-medium text-fg group-hover:text-accent">{page.label}</p><p className="mt-2 text-xs leading-5 text-muted group-hover:text-fg/80">{page.note}</p></Link>)}</div></div></Panel>
