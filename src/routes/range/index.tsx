    ...
    <div className="min-h-dvh bg-bg">
      ...
      <main className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
        <div className="max-w-3xl">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-green">
            VéLOXIS · The range
          </p>
          <h1 className="mt-3 text-5xl font-bold leading-none tracking-tight text-accent sm:text-6xl">
            Compare the range.
          </h1>
          <p className="mt-4 text-lg text-muted">
            Core · Pro · Apex. Choose two builds and compare the specification and BOM side-by-side.
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_1fr_320px]">
          ...
          <aside className="rounded-xl border border-border bg-bg-elevated p-5 lg:sticky lg:top-20 lg:self-start">
            <div className="flex items-baseline justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-green">BOM</p>
            ...
            <div className="divide-y divide-border">
              {bom.map(([name, x, y, flag]) => (
                <div key={String(name)} className="grid grid-cols-[1fr_64px_64px] gap-2 py-2.5 text-xs">
                  <span className="pr-2 leading-4 text-muted">
                    {String(name)}
                    {flag === "hs" ? <span className="ml-1 text-warn">*</span> : null}
                  </span>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 border-t border-border pt-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.14em] text-subtle">A total</p>
                <p className="mt-1 text-lg tabular-nums text-accent">{inr(ac)}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.14em] text-subtle">B total</p>
                <p className="mt-1 text-lg tabular-nums text-accent">{inr(bc)}</p>
              </div>
            </div>
...
function Spec({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="grid grid-cols-[96px_1fr] gap-4 py-4 text-sm">
      ...
      <dd className={strong ? "text-right font-semibold tabular-nums text-accent" : "text-right text-fg"}>{value}</dd>
    </div>
  );
}
