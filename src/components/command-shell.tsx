...
type NavGroup = { label: string; icon: typeof Activity; items: NavItem[]; defaultOpen?: boolean };

const GROUPS: NavGroup[] = [
  { label: "Overview", icon: Activity, defaultOpen: true, items: [
    { to: "/command", label: "Board", icon: Activity, exact: true },
    { to: "/command/financial-cockpit", label: "Financial Cockpit", icon: Wallet },
  ] },
  { label: "Finance", icon: Wallet, defaultOpen: true, items: [
  ...
  { label: "Production", icon: Factory, defaultOpen: true, items: [
  ...
  { label: "Product & Technical", icon: Wrench, items: [
  ...
  { label: "Sales & Market", icon: LineChart, items: [
  ...
  { label: "Legal & Risk", icon: Scale, items: [
  ...
  { label: "Leadership & Investors", icon: Presentation, items: [
  ...
  { label: "Knowledge & Delivery", icon: BookOpen, items: [
...
function Group({ group }: { group: NavGroup }) {
  ...
  return <details open={active || group.defaultOpen} className="group/section">
    <summary className="flex cursor-pointer list-none items-center gap-2 rounded-md px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-subtle hover:bg-surface hover:text-fg [&::-webkit-details-marker]:hidden">
      <group.icon className="size-3.5 shrink-0" /><span className="flex-1">{group.label}</span><ChevronDown className="size-3 transition-transform group-open/section:rotate-180" />
    </summary>
    <div className="ml-2 mt-1 space-y-0.5 border-l border-border pl-2">{group.items.map((item) => <Link key={item.to} to={item.to} activeOptions={item.exact ? { exact: true } : undefined} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-muted transition-colors duration-150 hover:bg-surface hover:text-fg" activeProps={{ className: "bg-surface text-fg" }}><item.icon className="size-4 shrink-0" />{item.label}</Link>)}</div>
  </details>;
}
