...
const EXECUTIVE_SUPPORT_ROUTES = new Set<string>(["/command/control-tower", "/command/management-intelligence", "/command/founder-command", "/command/founder-control", "/command/decision-engine"]);
const FINANCE_SUPPORT_ROUTES = new Set<string>(["/command/finance", "/command/balance-sheet", "/command/ca-audit", "/command/cash", "/command/finance-control", "/command/master-finance", "/command/aluminium-finance"]);
const FINANCE_TABS = [
...
const FINANCE_CONTEXT_ROUTES = new Set<string>([...FINANCE_TABS.map((tab) => tab.to), "/command/finance", "/command/finance-control", "/command/master-finance", "/command/aluminium-finance", "/command/funding"]);

const SUPPLY_SUPPORT_ROUTES = new Set<string>(["/command/procurement", "/command/inventory", "/command/production", "/command/manufacturing", "/command/quality", "/command/ops", "/command/actuals", "/command/production-jobcards"]);
const SUPPLY_TABS = [
...
const SUPPLY_CONTEXT_ROUTES = new Set<string>([...SUPPLY_TABS.map((tab) => tab.to), "/command/ops", "/command/actuals", "/command/production-jobcards", "/command/procurement-planning", "/command/inventory-truth", "/command/inventory-ledgers", "/command/inventory-master", "/command/inventory-openings", "/command/inventory-control-audit", "/command/component-control", "/command/inventory-legacy", "/command/bom-inventory-mapping", "/command/phase-6"]);
...
const COMMERCIAL_CONTEXT_ROUTES = new Set<string>([...COMMERCIAL_TABS.map((tab) => tab.to), "/command/phase-4"]);

const ENGINEERING_SUPPORT_ROUTES = new Set<string>(["/command/product", "/command/bom", "/command/bom-control"]);
...
const ENGINEERING_CONTEXT_ROUTES = new Set<string>([...ENGINEERING_TABS.map((tab) => tab.to), "/command/phase-5"]);

const GOVERNANCE_SUPPORT_ROUTES = new Set<string>(["/command/risk", "/command/legal", "/command/qa-verification", "/command/actions"]);
...
const PLAN_SUPPORT_ROUTES = new Set<string>(["/command/finance-assumptions", "/command/scenarios", "/command/procurement-planning", "/command/funding"]);
const PLAN_CONTEXT_ROUTES = new Set<string>([MASTER_PLAN_ROUTE, ...PLAN_SUPPORT_ROUTES]);
const LEGACY_ROUTES = new Set<string>(["/command/phase-4", "/command/phase-5", "/command/phase-6", "/command/phase-6a"]);
...
const SUPPORT_ROUTES = new Set<string>([...EXECUTIVE_SUPPORT_ROUTES, ...FINANCE_SUPPORT_ROUTES, ...SUPPLY_SUPPORT_ROUTES, ...COMMERCIAL_SUPPORT_ROUTES, ...ENGINEERING_SUPPORT_ROUTES, ...GOVERNANCE_SUPPORT_ROUTES, ...PLAN_SUPPORT_ROUTES]);
...
function PageLink({ page, role }: { page: RouteMeta; role: CommandRole | null }) {
  ...
  return (
    <Link to={page.route as never} title={page.label} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-muted transition-colors hover:bg-surface hover:text-fg" activeProps={{ className: "bg-surface text-fg" }}>
    ...
    <section className="rounded-xl border border-border bg-surface/30 p-2">
      ...
      <div className="space-y-0.5">
        {items.map((item) => {
          ...
          return (
            <Link key={item.to} to={item.to as never} activeOptions={item.exact ? { exact: true } : undefined} title={item.label} className={cn("flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium text-muted transition-colors hover:bg-bg hover:text-fg", active && "bg-bg text-fg shadow-sm")} activeProps={{ className: "bg-bg text-fg shadow-sm" }}>
...
function WorkspaceTabs({ routes, context, label, role }: { routes: readonly { to: string; label: string }[]; context: Set<string>; label: string; role: CommandRole | null }) {
  const location = useLocation();
    ...
    <nav className="mb-6 overflow-x-auto rounded-xl border border-border bg-surface/50 p-1 [scrollbar-width:thin]" aria-label={label}>
      <div className="flex min-w-max gap-1">
        {accessibleRoutes.map((tab) => (
          <Link key={tab.to} to={tab.to as never} className={cn("rounded-lg px-4 py-2 text-xs font-semibold transition-colors", location.pathname === tab.to ? "border border-accent/35 bg-accent/10 text-accent" : "border border-transparent text-muted hover:bg-bg/60 hover:text-fg")}>
            {tab.label}
          </Link>
        ))}
      </div>
    ...
    <section className="mb-5 rounded-xl border border-border bg-surface/35 p-3" aria-label="ERP business flow">
      ...
      <div className="mt-2 grid gap-2 text-[10px] sm:grid-cols-2">
        {previous && firstAccessible(previous.routes) ? (
          <Link to={firstAccessible(previous.routes)! as never} className="rounded-md border border-border px-2.5 py-1.5 text-muted hover:bg-bg hover:text-fg">
            ← {previous.label}
          </Link>
        ) : (
          <span />
        ...
        {next && firstAccessible(next.routes) ? (
          <Link to={firstAccessible(next.routes)! as never} className="rounded-md border border-border px-2.5 py-1.5 text-left text-muted hover:bg-bg hover:text-fg sm:text-right">
            {next.label} →
          </Link>
        ) : (
          <span />
...
function workspaceForPath(pathname: string) {
  ...
  return WORKSPACES.find((workspace) => route === workspace.to || workspace.context.has(route) || [...workspace.context].some((candidate) => candidate !== "/command" && route.startsWith(`${candidate}/`)));
}
...
function ContextBack({ role }: { role: CommandRole | null }) {
  ...
  const label = target === workspaceTarget ? (workspace?.label ?? "Command Centre") : "Command Centre";
  if (location.pathname === target) return null;
  return (
    <Link to={target as never} className="mb-4 inline-flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 text-xs font-medium text-muted transition-colors hover:border-accent/40 hover:bg-bg hover:text-fg" aria-label={`Return to ${label}`}>
      ← {label}
    </Link>
  );
}
    ...
    <div className="border-b border-border px-3 py-2 lg:hidden">
      <div className="flex items-center gap-2">
        <div className="grid min-w-0 flex-1 grid-cols-2 gap-1 rounded-md border border-border bg-surface/40 p-0.5">
          <button type="button" onClick={() => setView("workspaces")} className={cn("rounded px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em]", view === "workspaces" ? "bg-bg text-fg shadow-sm" : "text-muted hover:text-fg")}>
            Workspaces
          </button>
          <button type="button" onClick={() => setView("more")} className={cn("rounded px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em]", view === "more" ? "bg-bg text-fg shadow-sm" : "text-muted hover:text-fg")}>
            More
          </button>
        </div>
      ...
      <button type="button" onClick={logout} disabled={loggingOut} className="mt-2 w-full rounded-md border border-border px-3 py-2 text-xs text-muted hover:bg-surface hover:text-fg">
        ...
        <aside className="sticky top-14 hidden h-[calc(100dvh-3.5rem)] w-72 shrink-0 flex-col border-r border-border py-6 lg:flex">
          ...
          <div className="px-3 pb-3">
            <div className="grid grid-cols-2 rounded-md border border-border bg-surface/40 p-0.5">
              <button type="button" onClick={() => setView("workspaces")} className={cn("rounded px-2 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em]", view === "workspaces" ? "bg-bg text-fg shadow-sm" : "text-muted hover:text-fg")}>
                Workspaces
              </button>
              <button type="button" onClick={() => setView("more")} className={cn("rounded px-2 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em]", view === "more" ? "bg-bg text-fg shadow-sm" : "text-muted hover:text-fg")}>
                More
              </button>
            </div>
          </div>
